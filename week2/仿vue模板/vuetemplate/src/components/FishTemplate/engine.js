...
export default class Engine {
      ...
      if (pnode.attr.get("for")) {
        ...
        for (let i = 0; i < scope[prop].length; i++) {
          ...
          newnode.children.forEach((item) => {
            if (item.attr.has("v-if")) {
              let attr = item.attr.get("v-if")
              console.log('scope[prop][i][attr]', item, newScope)
              let scopeMap = attr.split('.')
              let name = newScope
              for (let i = 0; i < scopeMap.length; i++) {
                console.log('namename', scopeMap[i], name[scopeMap[i]])
                name = name[scopeMap[i]]
              }
              console.log('name', newScope, name)
              if (name) {
  ...
  parseAttribute(str) {
    let attr = new Map();
    str = str.trim();
    str.replace(/((v-)*\w+)?\s*=['"](.*?)['"]/gm, (s0, s1, s2, s3) => {
      console.log('s3', s0, s1, s2, s3)
      attr.set(s1, s3);
      return s0;
    });
    return attr;
